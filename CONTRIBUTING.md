...
1. Create `src/themes/<name>.ts` exporting a `Theme` with a full palette + three button colors. Copy `src/themes/nord.ts` as a starting point — every color slot needs a value (no defaults).
