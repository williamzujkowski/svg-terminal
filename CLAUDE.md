...
- `line-renderer.ts` — per-line SVG.
  ...
  - Output: simple opacity fade-in.
  - Animated blocks: N `<text>` siblings at the same y, each cycling opacity via a discrete `<animate>` over `frames.length / fps` seconds.
- `cache.ts` — JSON cache file (`{ version: 1, entries: { ... } }`). `flushCache` prunes entries older than `cacheTTL` before writing. `resolveCachePath` guards against `..`-traversal (string-based; symlinked configDir limitations tracked in issue #84). `makeUseCache` throws on `ttl < 0`.
